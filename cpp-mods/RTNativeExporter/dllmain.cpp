...
// BEGIN PHASE8K_ROOTCOMPONENT_PRECISION

static int32_t phase8k_i32(uint8_t* base, size_t off)
{
    int32_t v = 0;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static float phase8k_f32(uint8_t* base, size_t off)
{
    float v = 0.f;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static uintptr_t phase8k_ptr(uint8_t* base, size_t off)
{
    uintptr_t v = 0;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static bool phase8k_probably_ptr(uintptr_t v)
{
    if (v < 0x10000ULL) return false;
    if (v == 0xffffffffffffffffULL) return false;
    if (v == 0xccccccccccccccccULL) return false;
...
static bool phase8k_valid_float(float v)
{
    if (!std::isfinite(v)) return false;
    if (std::abs(v) > 200000000.0f) return false;
    return true;
...
static bool phase8k_valid_triplet(float x, float y, float z)
{
    return phase8k_valid_float(x) && phase8k_valid_float(y) && phase8k_valid_float(z);
}

static int phase8k_extract_int_after(const std::string& text, const std::string& token)
{
    size_t pos = text.rfind(token);
    if (pos == std::string::npos) return -1;
    pos += token.size();
...
static int phase8k_spacing(const std::set<int32_t>& values)
{
    if (values.size() < 2) return 0;
...
static bool phase8k_near_multiple(float v, int spacing)
{
    if (spacing <= 0) return false;
    if (!std::isfinite(v)) return false;

    float q = v / static_cast<float>(spacing);
    float nearest = std::round(q);
    float diff = std::abs(q - nearest);

    return diff < 0.01f;
}

static UObject* phase8k_find_function_by_name_contains(const std::string& needle)
{
    ...
    RC::Unreal::UObjectGlobals::ForEachUObject(
        [&](UObject* o, [[maybe_unused]] int32_t chunk_index, [[maybe_unused]] int32_t object_index)
        {
            ...
            if (cls.find("Function") != std::string::npos &&
                ...
                path.find("/Script/Engine.Actor:") != std::string::npos)
            {
                found = o;
            }
...
static void write_phase8k_rootcomponent_precision_snapshot(int run, int delay_seconds)
{
    auto out = out_dir();

    std::ofstream summary(out / "phase8k_summary.txt", std::ios::app);
    std::ofstream root_csv(out / "phase8k_rootcomponent_direct.csv", std::ios::app);
    std::ofstream scan_csv(out / "phase8k_rootcomponent_transform_scan.csv", std::ios::app);
    std::ofstream grid_csv(out / "phase8k_grid_match_candidates.csv", std::ios::app);
    std::ofstream bounds_csv(out / "phase8k_collision_island_bounds.csv", std::ios::app);
    std::ofstream k2_csv(out / "phase8k_k2_get_actor_location_candidates.csv", std::ios::app);

    if (run == 1)
    {
        root_csv << "run,delay_seconds,landscape_id,actor_name,actor_path,actor_addr,root_ptr,root_class,root_name,root_path,actor_spacing_x_1960,actor_spacing_y_1964,bounds_spacing_x,bounds_spacing_y,spacing_match\n";
        scan_csv << "run,delay_seconds,landscape_id,actor_name,root_addr,root_path,offset,x,y,z,i0,i4,i8,tag\n";
        grid_csv << "run,delay_seconds,landscape_id,actor_name,root_addr,offset,x,y,z,bounds_spacing_x,bounds_spacing_y,match_x,match_y,tag\n";
        bounds_csv << "run,delay_seconds,landscape_id,count,grid_x,grid_y,min_x,max_x,min_y,max_y,spacing_x,spacing_y,values_x,values_y\n";
        k2_csv << "run,delay_seconds,landscape_id,actor_name,actor_path,actor_addr,function_found,called,x,y,z,note\n";
    }
    ...
    RC::Unreal::UObjectGlobals::ForEachUObject(
        [&](UObject* o, [[maybe_unused]] int32_t chunk_index, [[maybe_unused]] int32_t object_index)
        {
            ...
            if (is_collision)
            {
                ...
                uint8_t* base = reinterpret_cast<uint8_t*>(o);
                int lid = phase8k_extract_int_after(path, "Landscape_");
                int32_t sx = phase8k_i32(base, 1296);
                int32_t sy = phase8k_i32(base, 1300);
            ...
            {
                Actor a{};
                a.landscape_id = phase8k_extract_int_after(path, "Landscape_");
                a.cls = cls;
                a.name = name;
                a.path = path;
                a.obj = o;
    ...
    for (const auto& kv : bounds)
    {
        ...
        int grid_x = static_cast<int>(b.xs.size());
        int grid_y = static_cast<int>(b.ys.size());
        int spacing_x = phase8k_spacing(b.xs);
        int spacing_y = phase8k_spacing(b.ys);
    ...
    UObject* k2_func = phase8k_find_function_by_name_contains("K2_GetActorLocation");
    ...
    for (const auto& a : actors)
    {
        uint8_t* actor_base = reinterpret_cast<uint8_t*>(a.obj);

        uintptr_t root_ptr = phase8k_ptr(actor_base, 440);
        int actor_spacing_x = phase8k_i32(actor_base, 1960);
        int actor_spacing_y = phase8k_i32(actor_base, 1964);
        ...
        auto it = object_by_addr.find(root_ptr);
        if (it != object_by_addr.end())
        {
            root_info = it->second;
            root_ok = true;
            root_found++;
        ...
        root_csv << run << ","
                 ...
                 << "\"" << a.path << "\","
                 << "\"0x" << std::hex << reinterpret_cast<uintptr_t>(a.obj) << std::dec << "\","
                 << "\"0x" << std::hex << root_ptr << std::dec << "\","
                 << "\"" << (root_ok ? root_info.cls : std::string("")) << "\","
                 << "\"" << (root_ok ? root_info.name : std::string("")) << "\","
                 << "\"" << (root_ok ? root_info.path : std::string("")) << "\","
                 << actor_spacing_x << ","
                 << actor_spacing_y << ","
                 << bounds_spacing_x << ","
                 << bounds_spacing_y << ","
        ...
        if (root_ok)
        {
            uint8_t* root_base = reinterpret_cast<uint8_t*>(root_info.obj);

            for (size_t off = 0x100; off <= 0x500; off += 4)
            {
                float x = phase8k_f32(root_base, off);
                float y = phase8k_f32(root_base, off + 4);
                float z = phase8k_f32(root_base, off + 8);

                if (!phase8k_valid_triplet(x, y, z))
                    continue;

                int32_t i0 = phase8k_i32(root_base, off);
                int32_t i4 = phase8k_i32(root_base, off + 4);
                int32_t i8 = phase8k_i32(root_base, off + 8);

                bool all_zero = std::abs(x) < 0.000001f && std::abs(y) < 0.000001f && std::abs(z) < 0.000001f;
                bool grid_x = phase8k_near_multiple(x, bounds_spacing_x) || phase8k_near_multiple(x, bounds_spacing_y);
                bool grid_y = phase8k_near_multiple(y, bounds_spacing_x) || phase8k_near_multiple(y, bounds_spacing_y);

                std::string tag = "triplet";
                if (all_zero) tag = "zero_triplet";
                if (grid_x || grid_y) tag = "grid_multiple_candidate";

                scan_csv << run << ","
                         << delay_seconds << ","
                         << a.landscape_id << ","
                         << "\"" << a.name << "\","
                         << "\"0x" << std::hex << reinterpret_cast<uintptr_t>(root_info.obj) << std::dec << "\","
                         << "\"" << root_info.path << "\","
                         << off << ","
                         << x << ","
                         << y << ","
        ...
        if (k2_func)
        {
            ...
            try
            {
                a.obj->ProcessEvent(reinterpret_cast<UFunction*>(k2_func), &params);
                called = true;
                note = "ProcessEvent_called";
                k2_success++;
            }
            catch (...)
            {
                called = false;
                note = "ProcessEvent_exception";
            }
        ...
        else
        {
            k2_csv << run << ","
                   ...
                   << "\"" << a.path << "\","
                   << "\"0x" << std::hex << reinterpret_cast<uintptr_t>(a.obj) << std::dec << "\","
                   << "0,0,0,0,0,\"function_not_found\"\n";
        }
    }

    summary << "\n===== PHASE 8K RUN " << run << " DELAY " << delay_seconds << "s =====\n";
    summary << "scanned_objects=" << scanned << "\n";
    summary << "collision_components=" << collisions << "\n";
    summary << "landscape_components_seen=" << components << "\n";
    summary << "islands_from_collision_bounds=" << bounds.size() << "\n";
    summary << "landscape_actors=" << actors.size() << "\n";
    summary << "confirmed_rootcomponent_actor_offset=440\n";
    summary << "confirmed_spacing_offset=1960/1964\n";
    summary << "root_components_found=" << root_found << "\n";
    summary << "root_scan_rows=" << scan_rows << "\n";
    summary << "grid_match_rows=" << grid_matches << "\n";
    summary << "k2_function_found=" << (k2_func ? "1" : "0") << "\n";
    summary << "k2_attempts=" << k2_attempts << "\n";
    summary << "k2_success=" << k2_success << "\n";

    if (bounds.empty() || collisions == 0 || actors.empty())
    {
        summary << "DECISION=skip_not_loaded\n";
        file_log("Phase 8K skip run=" + std::to_string(run));
        return;
    }

    summary << "DECISION=rootcomponent_precision_written\n";

    file_log("Phase 8K done run=" + std::to_string(run) +
             " actors=" + std::to_string(actors.size()) +
             " roots=" + std::to_string(root_found) +
             " grid_matches=" + std::to_string(grid_matches) +
             " k2_found=" + std::to_string(k2_func ? 1 : 0));
}

static void start_phase8k_rootcomponent_precision()
{
    static bool started = false;
    if (started) return;
    started = true;

    file_log("Phase 8K rootcomponent precision started");
    ...
    std::thread([]()
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            write_phase8k_rootcomponent_precision_snapshot(i + 1, target);
        }

        file_log("Phase 8K rootcomponent precision finished");
    }).detach();
}

// END PHASE8K_ROOTCOMPONENT_PRECISION





// BEGIN PHASE8L_WORLDMAP_EXPORT

static int32_t phase8l_i32(uint8_t* base, size_t off)
{
    int32_t v = 0;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static uintptr_t phase8l_ptr(uint8_t* base, size_t off)
{
    uintptr_t v = 0;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static int phase8l_extract_int_after(const std::string& text, const std::string& token)
{
    size_t pos = text.rfind(token);
    if (pos == std::string::npos) return -1;
    pos += token.size();
...
static int phase8l_spacing(const std::set<int32_t>& values)
{
    if (values.size() < 2) return 0;
...
static UObject* phase8l_find_k2_get_actor_location()
{
    UObject* found = nullptr;
    ...
    RC::Unreal::UObjectGlobals::ForEachUObject(
        [&](UObject* o, [[maybe_unused]] int32_t chunk_index, [[maybe_unused]] int32_t object_index)
        {
            ...
            if (cls.find("Function") != std::string::npos &&
                name == "K2_GetActorLocation" &&
                path.find("/Script/Engine.Actor:K2_GetActorLocation") != std::string::npos)
            {
                found = o;
            }
...
static void write_phase8l_worldmap_export_snapshot(int run, int delay_seconds)
{
    auto out = out_dir();

    std::ofstream summary(out / "phase8l_summary.txt", std::ios::app);
    std::ofstream islands_csv(out / "phase8l_worldmap_islands.csv", std::ios::app);
    std::ofstream comps_csv(out / "phase8l_worldmap_components.csv", std::ios::app);
    std::ofstream manifest(out / "phase8l_export_manifest.txt", std::ios::app);

    if (run == 1)
    {
        islands_csv
            << "run,delay_seconds,landscape_id,actor_name,actor_path,actor_addr,"
            << "world_x,world_y,world_z,"
            << "root_ptr,root_class,root_name,root_path,"
            << "actor_spacing_x_1960,actor_spacing_y_1964,"
            << "bounds_spacing_x,bounds_spacing_y,spacing_match,"
            << "component_count,grid_x,grid_y,min_section_x,max_section_x,min_section_y,max_section_y,"
            << "world_min_x_guess,world_max_x_guess,world_min_y_guess,world_max_y_guess,"
            << "export_status\n";

        comps_csv
            << "run,delay_seconds,landscape_id,component_id,component_type,"
            << "section_x,section_y,local_grid_x,local_grid_y,"
            << "island_world_x,island_world_y,island_world_z,"
            << "spacing_x,spacing_y,"
            << "component_world_x_guess,component_world_y_guess,"
            << "component_path,component_addr\n";
    }
    ...
    std::vector<Actor> actors;
    std::vector<Component> collision_components;
    std::map<int, Bounds> bounds;
    std::map<uintptr_t, ObjInfo> object_by_addr;
    ...
    RC::Unreal::UObjectGlobals::ForEachUObject(
        [&](UObject* o, [[maybe_unused]] int32_t chunk_index, [[maybe_unused]] int32_t object_index)
        {
            ...
            if (is_component) landscape_components_seen++;

            if (is_landscape_actor)
            {
                Actor a{};
                a.landscape_id = phase8l_extract_int_after(path, "Landscape_");
                a.name = name;
                a.path = path;
                a.obj = o;
                actors.push_back(a);
            ...
            if (is_collision)
            {
                ...
                Component c{};
                c.landscape_id = phase8l_extract_int_after(path, "Landscape_");
                c.component_id = phase8l_extract_int_after(path, "LandscapeHeightfieldCollisionComponent_");
                c.type = "collision";
                c.section_x = phase8l_i32(base, 1296);
                c.section_y = phase8l_i32(base, 1300);
                c.path = path;
                c.obj = o;
    ...
    std::sort(collision_components.begin(), collision_components.end(), [](const Component& a, const Component& b)
    {
        if (a.landscape_id != b.landscape_id) return a.landscape_id < b.landscape_id;
        if (a.section_x != b.section_x) return a.section_x < b.section_x;
        if (a.section_y != b.section_y) return a.section_y < b.section_y;
    ...
    UObject* k2_func = phase8l_find_k2_get_actor_location();
    ...
    struct IslandExport
    {
        ...
        int spacing_x = 0;
        int spacing_y = 0;
        int actor_spacing_x = 0;
        int actor_spacing_y = 0;
        int grid_x = 0;
        int grid_y = 0;
        int minx = 0;
        int maxx = 0;
    ...
    std::map<int, IslandExport> island_export;

    int k2_success = 0;
    int roots_found = 0;
    int islands_written = 0;
    int components_written = 0;

    for (const auto& a : actors)
    {
        uint8_t* actor_base = reinterpret_cast<uint8_t*>(a.obj);

        uintptr_t root_ptr = phase8l_ptr(actor_base, 440);
        int actor_spacing_x = phase8l_i32(actor_base, 1960);
        int actor_spacing_y = phase8l_i32(actor_base, 1964);

        ObjInfo root_info{};
        bool root_ok = false;

        auto rit = object_by_addr.find(root_ptr);
        if (rit != object_by_addr.end())
        {
            root_info = rit->second;
            root_ok = true;
            roots_found++;
        }
        ...
        if (bit != bounds.end())
        {
            ...
            bounds_spacing_x = phase8l_spacing(b.xs);
            bounds_spacing_y = phase8l_spacing(b.ys);
            grid_x = static_cast<int>(b.xs.size());
            grid_y = static_cast<int>(b.ys.size());
            minx = b.minx;
            maxx = b.maxx;
            miny = b.miny;
            maxy = b.maxy;
            component_count = b.count;
        }
        ...
        if (k2_func)
        {
            try
            {
                a.obj->ProcessEvent(reinterpret_cast<UFunction*>(k2_func), &loc);
                has_location = true;
                k2_success++;
            }
            catch (...)
        ...
        ie.wy = loc.Y;
        ie.wz = loc.Z;
        ie.has_location = has_location;
        ie.spacing_x = bounds_spacing_x;
        ie.spacing_y = bounds_spacing_y;
        ie.actor_spacing_x = actor_spacing_x;
        ie.actor_spacing_y = actor_spacing_y;
        ie.grid_x = grid_x;
        ie.grid_y = grid_y;
        ie.minx = minx;
        ie.maxx = maxx;
        ie.miny = miny;
        ie.maxy = maxy;
        island_export[a.landscape_id] = ie;

        double world_min_x_guess = loc.X + static_cast<double>(minx) * static_cast<double>(bounds_spacing_x);
        double world_max_x_guess = loc.X + static_cast<double>(maxx) * static_cast<double>(bounds_spacing_x);
        double world_min_y_guess = loc.Y + static_cast<double>(miny) * static_cast<double>(bounds_spacing_y);
        double world_max_y_guess = loc.Y + static_cast<double>(maxy) * static_cast<double>(bounds_spacing_y);

        std::string status = "ok";
        if (!has_location) status = "missing_k2_location";
        if (!spacing_match) status = "spacing_mismatch";
        if (component_count <= 0) status = "missing_components";
    ...
    for (const auto& c : collision_components)
    {
        auto iit = island_export.find(c.landscape_id);
        if (iit == island_export.end()) continue;

        const IslandExport& ie = iit->second;

        int local_grid_x = 0;
        int local_grid_y = 0;

        if (ie.spacing_x > 0) local_grid_x = (c.section_x - ie.minx) / ie.spacing_x;
        if (ie.spacing_y > 0) local_grid_y = (c.section_y - ie.miny) / ie.spacing_y;

        double component_world_x_guess =
            ie.wx + static_cast<double>(c.section_x) * static_cast<double>(ie.spacing_x);

        double component_world_y_guess =
            ie.wy + static_cast<double>(c.section_y) * static_cast<double>(ie.spacing_y);
        ...
        comps_csv << run << ","
                  ...
                  << ie.spacing_x << ","
                  << ie.spacing_y << ","
                  << component_world_x_guess << ","
                  << component_world_y_guess << ","
                  << "\"" << c.path << "\","
    ...
    summary << "\n===== PHASE 8L RUN " << run << " DELAY " << delay_seconds << "s =====\n";
    summary << "scanned_objects=" << scanned << "\n";
    summary << "landscape_actors=" << actors.size() << "\n";
    summary << "collision_components=" << collision_components.size() << "\n";
    summary << "landscape_components_seen=" << landscape_components_seen << "\n";
    summary << "islands_from_collision_bounds=" << bounds.size() << "\n";
    summary << "confirmed_sectionbase_offsets=collision+1296/+1300\n";
    summary << "confirmed_rootcomponent_actor_offset=440\n";
    summary << "confirmed_spacing_offset=1960/1964\n";
    summary << "k2_function_found=" << (k2_func ? "1" : "0") << "\n";
    summary << "k2_success=" << k2_success << "\n";
    summary << "roots_found=" << roots_found << "\n";
    summary << "islands_written=" << islands_written << "\n";
    summary << "components_written=" << components_written << "\n";
    summary << "DECISION=worldmap_export_written\n";

    manifest << "\n===== PHASE 8L EXPORT MANIFEST RUN " << run << " =====\n";
    manifest << "phase8l_worldmap_islands.csv = authoritative island/origin/grid export\n";
    manifest << "phase8l_worldmap_components.csv = component-level worldmap export using SectionBaseX/Y\n";
    manifest << "formula_component_world_x_guess = island_world_x + section_x * spacing_x\n";
    manifest << "formula_component_world_y_guess = island_world_y + section_y * spacing_y\n";
    manifest << "note = if rendered map appears scaled/offset, admin panel can normalize from exported world_min/max bounds\n";

    file_log("Phase 8L done run=" + std::to_string(run) +
             " islands=" + std::to_string(islands_written) +
             " components=" + std::to_string(components_written) +
             " k2=" + std::to_string(k2_success));
}

static void start_phase8l_worldmap_export()
{
    static bool started = false;
    if (started) return;
    started = true;

    file_log("Phase 8L worldmap export started");
    ...
    std::thread([]()
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            write_phase8l_worldmap_export_snapshot(i + 1, target);
        }

        file_log("Phase 8L worldmap export finished");
    }).detach();
}

// END PHASE8L_WORLDMAP_EXPORT
...
// BEGIN PHASE9I_RANGE_RECORD_HUNT

static uint32_t phase9i_u32(uint8_t* base, size_t off)
{
    uint32_t v = 0;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static int32_t phase9i_i32(uint8_t* base, size_t off)
{
    int32_t v = 0;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static uint16_t phase9i_u16(uint8_t* base, size_t off)
{
    uint16_t v = 0;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static float phase9i_f32(uint8_t* base, size_t off)
{
    float v = 0.0f;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static double phase9i_f64(uint8_t* base, size_t off)
{
    double v = 0.0;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static uintptr_t phase9i_ptr(uint8_t* base, size_t off)
{
    uintptr_t v = 0;
    std::memcpy(&v, base + off, sizeof(v));
    return v;
...
static bool phase9i_probably_ptr(uintptr_t v)
{
    if (v < 0x10000ULL) return false;
    if (v == 0xffffffffffffffffULL) return false;
    if (v == 0xccccccccccccccccULL) return false;
...
static bool phase9i_probably_subsystem(const std::string& name, const std::string& path, const std::string& cls)
{
    return name.find("VolumizationSubsystem") != std::string::npos ||
           path.find("VolumizationSubsystem") != std::string::npos ||
           cls.find("VolumizationSubsystem") != std::string::npos;
...
static void phase9i_hex_bytes(std::ofstream& os, uint8_t* base, size_t len)
{
    static const char* hexdigits = "0123456789abcdef";
    for (size_t i = 0; i < len; ++i)
    {
        unsigned char b = base[i];
        os << hexdigits[(b >> 4) & 0xf] << hexdigits[b & 0xf];
    }
}

static int phase9i_score_candidate(uint8_t* base)
{
    uint32_t entry_count = phase9i_u32(base, 0x150);
    uint32_t table_w = phase9i_u32(base, 0x154);
    uint32_t table_h = phase9i_u32(base, 0x158);
    uintptr_t entry_base = phase9i_ptr(base, 0x168);
    uintptr_t offset_array = phase9i_ptr(base, 0x178);
    uintptr_t value_array = phase9i_ptr(base, 0x188);
    uintptr_t prefix_array = phase9i_ptr(base, 0x198);

    int score = 0;
    if (entry_count > 0 && entry_count < 100000) score += 4;
    if (table_w > 100 && table_w < 100000) score += 3;
    if (table_h > 100 && table_h < 100000) score += 3;
    if (phase9i_probably_ptr(entry_base)) score += 4;
    if (phase9i_probably_ptr(offset_array)) score += 3;
    if (phase9i_probably_ptr(value_array)) score += 3;
    if (phase9i_probably_ptr(prefix_array)) score += 3;
    return score;
}

static void write_phase9i_range_record_hunt_snapshot(int run, int delay_seconds)
{
    auto out = out_dir();

    std::ofstream summary(out / "phase9i_summary.txt", std::ios::app);
    std::ofstream candidates(out / "phase9i_candidates.csv", std::ios::app);
    std::ofstream entrymap(out / "phase9i_entry_range_map.csv", std::ios::app);
    std::ofstream ranges(out / "phase9i_possible_range_records.csv", std::ios::app);
    std::ofstream scans(out / "phase9i_stride_scan_candidates.csv", std::ios::app);
    std::ofstream raw(out / "phase9i_range_raw_windows.csv", std::ios::app);
    std::ofstream notes(out / "phase9i_notes.txt", std::ios::app);

    if (run == 1)
    {
        candidates << "run,delay_seconds,name,path,class,addr,score,entry_count,table_width,table_height,entry_base,offset_array,value_array,prefix_array,prefix_sum\n";
        entrymap << "run,delay_seconds,subsystem_addr,index,range_start,range_count,range_end,center_x,center_y,grid_size,offset_x,offset_y,value_class\n";
        ranges << "run,delay_seconds,subsystem_addr,source,label,record_index,record_addr,stride,w0,w1,w2,w3,w4,w5,w6,w7,i0,i1,i2,i3,i4,i5,i6,i7,f0,f1,f2,f3\n";
        scans << "run,delay_seconds,subsystem_addr,source,label,base_addr,stride,records_tested,score,reason\n";
        raw << "run,delay_seconds,subsystem_addr,source,label,base_addr,byte_offset,window_hex\n";
    }
    ...
    RC::Unreal::UObjectGlobals::ForEachUObject(
        [&](UObject* o, [[maybe_unused]] int32_t chunk_index, [[maybe_unused]] int32_t object_index)
        {
            ...
            if (phase9i_probably_subsystem(name, path, cls))
            {
                uint8_t* base = reinterpret_cast<uint8_t*>(o);
                int score = phase9i_score_candidate(base);
                ObjInfo x{o, cls, name, path, score};
                candidates_vec.push_back(x);
            }
    ...
    std::sort(candidates_vec.begin(), candidates_vec.end(), [](const ObjInfo& a, const ObjInfo& b){ return a.score > b.score; });
    ...
    int raw_rows = 0;
    int entry_rows = 0;
    int best_score = candidates_vec.empty() ? 0 : candidates_vec.front().score;
    uintptr_t best_addr = candidates_vec.empty() ? 0 : reinterpret_cast<uintptr_t>(candidates_vec.front().obj);
    ...
    for (const auto& c : candidates_vec)
    {
        ...
        if (phase9i_probably_ptr(prefix_array) && entry_count > 0 && entry_count < 100000)
        {
            for (uint32_t i = 0; i < entry_count; ++i)
            {
                int32_t v = 0;
                std::memcpy(&v, reinterpret_cast<void*>(prefix_array + static_cast<uintptr_t>(i) * 4ULL), sizeof(v));
                if (v > 0 && v < 1000000) prefix_sum += static_cast<uint32_t>(v);
            }
        }
        ...
        if (entry_count == 0 || entry_count > 100000) continue;
        if (!phase9i_probably_ptr(entry_base)) continue;
        valid++;
        ...
        {
            int32_t count = 0;
            if (phase9i_probably_ptr(prefix_array))
                std::memcpy(&count, reinterpret_cast<void*>(prefix_array + static_cast<uintptr_t>(idx) * 4ULL), sizeof(count));

            if (count < 0) count = 0;
            if (count > 1024) count = 1024;

            starts.push_back(accum);
            counts.push_back(static_cast<uint32_t>(count));

            uintptr_t eaddr = entry_base + static_cast<uintptr_t>(idx) * 0x60ULL;
            uint8_t* eb = reinterpret_cast<uint8_t*>(eaddr);

            double cx = phase9i_f64(eb, 0x20);
            double cy = phase9i_f64(eb, 0x28);
            double gs = phase9i_f64(eb, 0x40);

            int32_t ox = 0;
            int32_t oy = 0;
            if (phase9i_probably_ptr(offset_array))
            {
                uint64_t packed = 0;
                std::memcpy(&packed, reinterpret_cast<void*>(offset_array + static_cast<uintptr_t>(idx) * 8ULL), sizeof(packed));
                ox = static_cast<int32_t>(packed & 0xffffffffULL);
            ...
            int32_t val = 0;
            if (phase9i_probably_ptr(value_array))
                std::memcpy(&val, reinterpret_cast<void*>(value_array + static_cast<uintptr_t>(idx) * 4ULL), sizeof(val));
        ...
        if (phase9i_probably_ptr(entry_base))
        {
            sources.push_back({"entry_array_plus_0x20", "entry_core_doubles", entry_base + 0x20, static_cast<size_t>(entry_count) * 0x60ULL});
            sources.push_back({"entry_array_plus_0x40", "entry_core_grid", entry_base + 0x40, static_cast<size_t>(entry_count) * 0x60ULL});
            sources.push_back({"entry_array_after_entries", "after_entry_array", entry_base + static_cast<uintptr_t>(entry_count) * 0x60ULL, 0x4000});
            if (entry_base > 0x4000)
                sources.push_back({"entry_array_before_entries", "before_entry_array", entry_base - 0x4000, 0x4000});
        }

        if (phase9i_probably_ptr(offset_array))
        {
            sources.push_back({"offset_array", "offset_int2_array", offset_array, static_cast<size_t>(entry_count) * 8ULL});
            sources.push_back({"offset_array_after", "after_offset_array", offset_array + static_cast<uintptr_t>(entry_count) * 8ULL, 0x4000});
            if (offset_array > 0x4000)
                sources.push_back({"offset_array_before", "before_offset_array", offset_array - 0x4000, 0x4000});
        }

        if (phase9i_probably_ptr(value_array))
        {
            sources.push_back({"value_array", "value_class_array", value_array, static_cast<size_t>(entry_count) * 4ULL});
            sources.push_back({"value_array_after", "after_value_array", value_array + static_cast<uintptr_t>(entry_count) * 4ULL, 0x4000});
            if (value_array > 0x4000)
                sources.push_back({"value_array_before", "before_value_array", value_array - 0x4000, 0x4000});
        }

        if (phase9i_probably_ptr(prefix_array))
        {
            sources.push_back({"prefix_array", "prefix_count_array", prefix_array, static_cast<size_t>(entry_count) * 4ULL});
            sources.push_back({"prefix_array_after", "after_prefix_array", prefix_array + static_cast<uintptr_t>(entry_count) * 4ULL, 0x8000});
            if (prefix_array > 0x8000)
                sources.push_back({"prefix_array_before", "before_prefix_array", prefix_array - 0x8000, 0x8000});
        }

        const int strides[] = {4, 8, 12, 16, 20, 24, 32, 40, 48, 64};

        for (const auto& src : sources)
        {
            if (!phase9i_probably_ptr(src.base_addr)) continue;

            for (int stride : strides)
            {
                if (src.max_bytes < static_cast<size_t>(stride)) continue;

                uint32_t records = static_cast<uint32_t>(src.max_bytes / static_cast<size_t>(stride));
                ...
                for (uint32_t ri = 0; ri < records; ++ri)
                {
                    uint8_t* rb = reinterpret_cast<uint8_t*>(src.base_addr + static_cast<uintptr_t>(ri) * static_cast<uintptr_t>(stride));
                    int32_t a = phase9i_i32(rb, 0);
                    int32_t b = stride >= 8 ? phase9i_i32(rb, 4) : 0;
                    int32_t c0 = stride >= 12 ? phase9i_i32(rb, 8) : 0;
                    float fa = phase9i_f32(rb, 0);
                    float fb = stride >= 8 ? phase9i_f32(rb, 4) : 0.0f;

                    if (a != 0 || b != 0 || c0 != 0) nonzero++;
                    if (a >= 0 && a <= 1261 && b >= 0 && b <= 1261) small_count++;
                    if ((fa > -2000000.0f && fa < 2000000.0f && fb > -2000000.0f && fb < 2000000.0f) && (fa != 0.0f || fb != 0.0f)) coord_like++;
                }

                score = small_count * 3 + coord_like + nonzero;

                std::string reason = "";
                if (small_count > 10) reason += "many_table_coord_pairs;";
                if (coord_like > 10) reason += "many_float_coord_pairs;";
                if (nonzero > 10) reason += "many_nonzero_records;";
                ...
                if (score > 40) should_dump = true;
                if (std::string(src.source).find("after") != std::string::npos && score > 20) should_dump = true;
                if (std::string(src.source) == "prefix_array_after") should_dump = true;
                if (std::string(src.source) == "offset_array_after") should_dump = true;

                if (!should_dump) continue;

                uint32_t dump_records = records;
                if (dump_records > 160) dump_records = 160;

                for (uint32_t ri = 0; ri < dump_records; ++ri)
                {
                    uint8_t* rb = reinterpret_cast<uint8_t*>(src.base_addr + static_cast<uintptr_t>(ri) * static_cast<uintptr_t>(stride));

                    uint32_t w[8] = {0,0,0,0,0,0,0,0};
                    int32_t ii[8] = {0,0,0,0,0,0,0,0};
                    float ff[4] = {0,0,0,0};
                    ...
                    for (int k = 0; k < 8; ++k)
                    {
                        if ((k + 1) * 4 <= stride)
                        {
                            w[k] = phase9i_u32(rb, k * 4);
                            ii[k] = phase9i_i32(rb, k * 4);
                        }
                    }
                    ...
                    ranges << run << ","
                           ...
                           << "\"" << src.label << "\","
                           << std::dec << ri << ","
                           << "\"0x" << std::hex << (src.base_addr + static_cast<uintptr_t>(ri) * static_cast<uintptr_t>(stride)) << "\","
                           << std::dec << stride;

                    for (int k = 0; k < 8; ++k)
                        ranges << ",\"0x" << std::hex << w[k] << "\"";
            ...
            size_t window_bytes = src.max_bytes;
            if (window_bytes > 0x400) window_bytes = 0x400;
            for (size_t bo = 0; bo < window_bytes; bo += 0x40)
            {
                raw << run << ","
                    << delay_seconds << ","
                    << "\"0x" << std::hex << reinterpret_cast<uintptr_t>(c.obj) << "\","
                ...
                phase9i_hex_bytes(raw, reinterpret_cast<uint8_t*>(src.base_addr + bo), 0x40);
    ...
    notes << "\n===== PHASE 9I NOTES RUN " << run << " =====\n";
    notes << "9I hunts for the next-level 133 range/slice records implied by Phase 9H prefix_sum.\n";
    notes << "It dumps stride scans around entry_array/offset_array/value_array/prefix_array and before/after adjacent memory windows.\n";
    notes << "Likely useful outputs: phase9i_entry_range_map.csv, phase9i_stride_scan_candidates.csv, phase9i_possible_range_records.csv, phase9i_range_raw_windows.csv.\n";
    notes << "This phase is read-only. No ProcessEvent, no GPU readback, no texture modification.\n";

    summary << "\n===== PHASE 9I RUN " << run << " DELAY " << delay_seconds << "s =====\n";
    summary << "scanned_objects=" << scanned_objects << "\n";
    summary << "subsystem_candidates=" << candidates_vec.size() << "\n";
    summary << "valid_like_subsystems=" << valid << "\n";
    summary << "best_score=" << best_score << "\n";
    summary << "best_addr=0x" << std::hex << best_addr << std::dec << "\n";
    summary << "entry_range_rows=" << entry_rows << "\n";
    summary << "stride_scan_rows=" << scan_rows << "\n";
    summary << "possible_range_record_rows=" << range_rows << "\n";
    summary << "raw_window_rows=" << raw_rows << "\n";

    if (valid > 0 && range_rows > 0)
        summary << "DECISION=range_record_hunt_dumped_candidates\n";
    else if (valid > 0)
        summary << "DECISION=valid_subsystem_found_no_range_candidates\n";
    else
        summary << "DECISION=no_valid_volumization_subsystem_found\n";

    file_log("Phase 9I done run=" + std::to_string(run) +
             " valid=" + std::to_string(valid) +
             " entry_rows=" + std::to_string(entry_rows) +
             " scan_rows=" + std::to_string(scan_rows) +
             " range_rows=" + std::to_string(range_rows));
}

static void start_phase9i_range_record_hunt()
{
    static bool started = false;
    if (started) return;
    started = true;

    file_log("Phase 9I range record hunt started");
    ...
    std::thread([]()
    {
        ...
        for (int i = 0; i < 2; ++i)
        {
            ...
            write_phase9i_range_record_hunt_snapshot(i + 1, target);
        }

        file_log("Phase 9I range record hunt finished");
    }).detach();
}

// END PHASE9I_RANGE_RECORD_HUNT
...
public:
    ...
    {
        Output::send<LogLevel::Verbose>(STR("[RTN] RTNativeExporter v1.32.0.2.2 on_unreal_init\n"));
        file_log("RTNativeExporter v1.32.0.2.2 on_unreal_init");
        ...
        start_phase9i_range_record_hunt();
